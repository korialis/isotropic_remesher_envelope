...
private:
    ...
    static bool is_outside(const CoordType & p0, const CoordType & p1, const Params & params) {
        const Vector3 point0 {p0[0], p0[1], p0[2]};
        const Vector3 point1 {p1[0], p1[1], p1[2]};

        return params.envelope.is_outside(point0, point1);
    }

    static bool is_outside(const CoordType & p, const Params & params) {
        const Vector3 point {p[0], p[1], p[2]};

        return params.envelope.is_outside(point);
    }


    static bool is_outside(const CoordType & p0, const CoordType & p1, const CoordType & p2, const Params & params) {
        ...
        return params.envelope.is_outside(triangle);
    }


    static bool is_outside(const FaceType & f, const Params & params) {
        
        const std::array<Vector3, 3> triangle {
            { f.cP(0)[0], f.cP(0)[1], f.cP(0)[2] },
            { f.cP(1)[0], f.cP(1)[1], f.cP(1)[2] },
            { f.cP(2)[0], f.cP(2)[1], f.cP(2)[2] }
        };

        return params.envelope.is_outside(triangle);
    }

    static void initEnvelope(MeshType & toRemesh, Params & params) {
        std::vector<Vector3> verts(toRemesh.VN());
        std::vector<Vector3i> faces(toRemesh.FN());

        std::cout << "building envelope" << std::endl;
        for (int i = 0; i < toRemesh.VN(); ++i) {
            const auto & p = toRemesh.vert[i].cP();
            verts[i] = {p[0], p[1], p[2]};
        }

        for (int i = 0; i < toRemesh.FN(); ++i) {
            const auto & f = toRemesh.face[i];
            faces[i] = {
                int(vcg::tri::Index(toRemesh, f.V(0))),
                int(vcg::tri::Index(toRemesh, f.V(1))),
                int(vcg::tri::Index(toRemesh, f.V(2)))
            };
        }

        params.envelope.init(verts, faces, params.maxSurfDist);
        std::cout << "envelope built" << std::endl;
    }
...
//                            face::CheckFlipEdge(f, i) &&
                            ...
                            (!params.surfDistCheck || !is_outside(swapEdgeMidPoint, params)))
                    {
    ...
    static bool checkCollapseFacesAroundVert1(const PosType &p, VertexPair & pair, Point3<ScalarType> &mp, Params &params, bool relaxed)
    {
        ...
        if (!checkFacesAfterCollapse(ff0, p0, mp, params, relaxed) || !checkFacesAfterCollapse(ff1, p1, mp, params, relaxed)) {
            return false;
        }

        if (params.surfDistCheck) {
            std::unordered_set<VertexPointer> starAroundCollapse;
            for (const auto fp : ff0) {
                const PosType pi(fp, p0.V()); //same vertex

                const auto v0 = pi.V();
                const auto v1 = pi.F()->V1(pi.VInd());
                const auto v2 = pi.F()->V2(pi.VInd());

                if( v1 == p0.VFlip() || v2 == p0.VFlip()) //i'm the other deleted face
                    continue;

                starAroundCollapse.insert(v1);
                starAroundCollapse.insert(v2);
            }
            for (const auto fp : ff1) {
                const PosType pi(fp, p1.V()); //same vertex

                const auto v0 = pi.V();
                const auto v1 = pi.F()->V1(pi.VInd());
                const auto v2 = pi.F()->V2(pi.VInd());

                if( v1 == p1.VFlip() || v2 == p1.VFlip()) //i'm the other deleted face
                    continue;
            ...
            return std::none_of(std::execution::par, starAroundCollapse.begin(), starAroundCollapse.end(), [&](const auto vp) {
                return is_outside(vp->cP(), mp, params);
            });
        }
